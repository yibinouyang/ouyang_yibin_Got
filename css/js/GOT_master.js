...
var initLhmVideo = function () {
	lhm_video = document.getElementById('lhm_video');
}

//Get the number of seconds that have been played
function getCurrentTime() {
	alert(parseInt(lhm_video.currentTime) + '：second');
}
...
function playOrPaused(obj) {
	if (lhm_video.paused) {
		...
		obj.firstElementChild.src="images/play.jpg";
		return;
	}
	lhm_video.pause();
	obj.innerHTML = 'play';
...
function vol(type, obj) {
	if (type == 'up') {
		var volume = lhm_video.volume + 0.1;
		if (volume >= 1) {
			volume = 1;

		}
		lhm_video.volume = volume;
	} else if (type == 'down') {
		var volume = lhm_video.volume - 0.1;
		if (volume <= 0) {
			volume = 0;
	...
	document.getElementById('nowVol').innerHTML = returnFloat1(lhm_video.volume);
}

//Mute
function muted(obj) {
	if (lhm_video.muted) {
		lhm_video.muted = false;
		console.log(obj.firstElementChild);
		obj.firstElementChild.src="images/voice1.jpg";
	} else {
		lhm_video.muted = true;
		console.log(obj.firstElementChild);
		obj.firstElementChild.src="images/voice.jpg";
	}
}
...
// Video rendering monitor video playback ends
lhm_video.addEventListener('play', function () {
...
//Monitor playback time, specific to milliseconds
var vkey1 = true;
var vkey2 = true;
lhm_video.addEventListener("timeupdate", function () {
	var timeDisplay;
	//Display the current playback progress in seconds
	timeDisplay = lhm_video.currentTime;
